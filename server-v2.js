...
async function requireApiKey(req, res, next) {
  ...
  // ── Trial expiry check ──────────────────────────────────────
  // If the client is on a trial that has expired, block immediately.
  // The scheduled expire_trials() DB function handles the actual plan downgrade,
  // but we enforce the block here in real-time for clients that slip through
  // before the next scheduled run.
  if (client.trial_ends_at) {
    const trialEnd = new Date(client.trial_ends_at);
    if (trialEnd < new Date()) {
      // Trial expired — fire-and-forget downgrade in case cron hasn't run yet
...
app.post('/api/v1/chat/completions', requireApiKey, async (req, res) => {
  ...
  const gate = checkBillingGate(client, plan);
  if (!gate.allowed) {
  ...
  const provider = providerRegistry.get(classifyResult.model)
